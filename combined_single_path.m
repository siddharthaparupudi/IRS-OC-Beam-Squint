...
rates_greedy = zeros(length(K_set),1);         % the average rate acheived
rates_RR = zeros(length(K_set),1);             % the average rate acheived by the RR scheme
max_rates_greedy = zeros(length(K_set),1);     % the maximum rate achievable (BF on all subcarriers)
...
for index = 1:length(K_set)
    ...
    rates_greedy(index) = avg_rate_greedy;
    rates_RR(index) = avg_rate_RR;
    max_rates_greedy(index) = (1-delta)*W*log2(1+(P/(No*N))*((M^2*P_alpha*P_beta*(1-epsilon))/(exp(1)*d_BS_IRS^(pathloss_BS_IRS)*d_IRS_UE^(pathloss_IRS_users)))*((0.7498*log(K))^(1.71) + 346.474*0.5772));
...
figure('Color', 'w');
semilogx(K_set, rates_RR, "-x");
hold on;
semilogx(K_set, rates_greedy,"-o");
hold on;
semilogx(K_set, max_rates_greedy,"-*");
xlim([min(K_set), max(K_set)]);
ylim([0, 1.2*max(max(max(max_rates_greedy), max(rates_RR)), max(rates_greedy))]);
