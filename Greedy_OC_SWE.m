...
alpha = zeros(L1,1);
for l1 = 1:L1
    alpha(l1) = 1e3/(sqrt(d_BS_IRS))^(pathloss_BS_IRS)*exp(-l1^2/2);
end
...
for k = 1:K
    ...
    for l2 = 1:L2_K(k)
        beta{k}(l2) = 1e6/(sqrt(d_IRS_users(k)))^(pathloss_IRS_users)*exp(-l2^2/2);
    end
end
...
for t = 1:T
    ...
    % BF phase configuration for the 1st path (i.e LoS path)
    ...
    for k = 1:K
        for i = 1:length(f)
            ...
            for m = 1:M
                h_k_matrix = h_k_matrix + exp(1i*phi(m))*gamma_C{k}.*exp(-1i*2*pi*f(i)*(tau_C{k})).*exp(-1i*2*pi*(m-1)*psi_C{k}*(1+f(i)/f_c));
            end
...
avg_gain_squared = gain_squared/(T*N);
