...
T = 50000;      % the number of time slots
tau = 5000;     % the time constant for the long term rates
...
for index = 1:length(K_set)
    for index_n = 1:length(N_set)
        for index_m = 1:length(M_set)
            ...
            max_rates_greedy(index, index_m, index_n) = (1/W)*(1-delta)*W*log2(1+(P/(No*N))*((M^2*P_alpha*P_beta*(1-epsilon))/(exp(1)*d_BS_IRS^(pathloss_BS_IRS)*d_IRS_UE^(pathloss_IRS_users)))*((0.7498*log(K))^(1.71) + 1.1));
...
% plot variation of average rate with number of IRS elements
figure('Color', 'w');
for index_n = 1:length(N_set)
    hold on;
